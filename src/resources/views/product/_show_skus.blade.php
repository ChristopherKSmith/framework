<div class="card card-accent-secondary">

        <div class="card-header">
           {{ __('Variants / SKUs') }}

            <div class="card-actionbar">
                @can('create products')
                    <a href="{{ route('vanilo.product_sku.create', $product) }}" class="btn btn-sm btn-outline-success float-right">
                        <i class="zmdi zmdi-plus"></i>
                        {{ __('New Product SKU') }}
                    </a>
                @endcan
            </div>

        </div>
        <div class="card-block">

            <table class="table table-striped table-hover">
                    <thead>
                        <tr>
                            <th>&nbsp;</th>
                            <th>{{ __('SKU') }}</th>
                            <th>{{ __('Cost') }}</th>
                            <th>{{ __('Price') }}</th>
                            <th>{{ __('Stock') }}</th>
                            <th style="width: 10%">&nbsp;</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php $defaultThumbnail = 'data:image/png;base64,'; ?>
                        @foreach($skus as $sku)
                            <tr>
                                <td>
                                    @can('view products')<a href="{{ route('vanilo.product.show', $product) }}">@endcan
                                        <img src="{{ $sku->getThumbnailUrl() ?: $defaultThumbnail }}" class="mw-100" style="height: 2.5em;" />
                                    @can('view products')</a>@endcan
                                </td>
                                <td>
                                    <span class="mb-3">
                                        {{$sku->code}}
                                    </span>
                                    @foreach($sku->propertyValues as $value)
                                        <div class="text-muted" title="">{{ $value->property->name }}: {{ $value->title }}</div>
                                    @endforeach
                                </td>
                                <td>
                                    <span class="mb-3">
                                        {{$sku->cost}}
                                    </span>
                                </td>
                                <td>
                                    <span class="mb-3">
                                        {{$sku->price}}
                                    </span>
                                    
                                    
                                </td>
                                <td>
                                    <span class="mb-3">
                                        {{$sku->stock}}
                                    </span>
                                    
                                    
                                </td>
                                <td>
                                    <a href="{{route('vanilo.product_sku.edit', [$product, $sku])}}" class="btn btn-xs btn-outline-primary btn-show-on-tr-hover float-right">{{ __('Edit') }}</a>
                                    @can('delete products')
                                        {!! Form::open(['url'  => route('vanilo.product_sku.update', [$product, $sku]),
                                                'method' => 'DELETE',
                                                'data-confirmation-text' => __('Are you sure to delete :name?', ['name' => $product->name])
                                            ])
                                        !!}
                                        <button class="btn btn-xs btn-outline-danger btn-show-on-tr-hover float-right">{{ __('Delete') }}</button>
                                        {!! Form::close() !!}
                                    @endcan
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
                
    
                @if($skus->hasPages())
                    <hr>
                    <nav>
                        {{ $skus->links() }}
                    </nav>
                @endif
        </div>
    </div>
    